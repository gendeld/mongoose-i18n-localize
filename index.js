...
function ArrNoDupe(a) {
  ...
  for (k in temp) {
    temp.hasOwnProperty(k) && r.push(k);
  }
  return r;
}
...
module.exports = function(schema, options) {
  ...
  var recursiveIteration = function(schema) {
    for (var key in schema.paths) {
      if (schema.paths[key].schema)
        recursiveIteration(schema.paths[key].schema);
      else addLocales(schema.paths[key].path, schema);
    }
  };

  if (options && options_locales instanceof Array && options_locales.length > 0)
  ...
  var localize = function(obj, locale, toJSON) {
    ...
    if (obj instanceof Array)
      return obj.map(function(object) {
        return addLocalized(
          toJSON ? object.toJSON() : object.toObject(),
          locale,
        );
      });
    ...
    var addLocalized = function(obj) {
      ...
      for (var key in obj) {
        ...
        if (key === '_id') continue;
          ...
          obj.localized = Array.isArray(obj[key]) ? obj[key][0] : obj[key];
